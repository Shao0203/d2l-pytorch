...
torch.save(model.state_dict(), 'model_weights.pth')  # save model params

new_model = NeuralNetwork()
new_model.load_state_dict(torch.load('model_weights.pth', weights_only=True))  # load model params
'''
...
with torch.no_grad():
    x = x.to(device)
    pred = new_model(x)
    print(f'Predicted: {pred.argmax(1).item()}, Actual: {y}')  # Predicted: 9, Actual: 9
